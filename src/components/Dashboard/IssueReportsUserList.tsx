...
const IssueReportsUserList: React.FC = () => {
    ...
    const fetchReports = useCallback(async () => {
        try {
            ...
            if (userRole === 'DISPATCH') {
                const dispatchReportedIssues = response.data.filter(issue => issue?.reportedBy?.role === 'DISPATCH' && issue?.reportedBy?.id === authenticatedUser?.id);
                filteredResult = [...new Set([...filteredResult, ...dispatchReportedIssues])];
            } else if (userRole === 'BENEWISE') {
                const benewiseReportedIssues = response.data.filter(issue => issue?.reportedBy?.role === 'BENEWISE' && issue?.reportedBy?.id === authenticatedUser?.id);
    ...
    const filterReports = useCallback(debounce(() => {
        let filtered = reports.filter(report =>
            ...
            (!filters.heavyEquipmentId || report.heavyEquipmentId === filters.heavyEquipmentId) &&
    ...
    return (
        <div className="p-4">
            ...
            <div className="mb-4 space-y-4">
                ...
                />
                <select 
                    name="heavyEquipmentId" 
                    className="mr-2 p-2 border border-gray-300" 
                    value={filters.heavyEquipmentId} 
                    onChange={handleFilterChange}>
                    ...
                    <table className="min-w-full bg-white border border-gray-300 print:border-none print:min-w-0">
                        <thead className="print:bg-transparent">
                            <tr>
                                ...
                                <th className="py-2 px-4 border-b print:border-none">Truck ID</th>
