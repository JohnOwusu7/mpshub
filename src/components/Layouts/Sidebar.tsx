...
const Sidebar = () => {
    ...
    const user = useSelector((state: IRootState) => state.user.user); // Fetch user from Redux store
            ...
            if (item.children) {
                ...
                return (
                    <li key={index} className="menu nav-item">
                        <button type="button" className={`${currentMenu === item.label.toLowerCase() ? 'active' : ''} nav-link group w-full`} onClick={() => toggleMenu(item.label.toLowerCase())}>
                            <div className="flex items-center">
                                {item.icon}
                                <span className="ltr:pl-3 rtl:pr-3 text-black dark:text-[#506690] dark:group-hover:text-white-dark">{t(item.label)}</span>
                            </div>
                            <div className={currentMenu !== item.label.toLowerCase() ? 'rtl:rotate-90 -rotate-90' : ''}>
                                <IconCaretDown />
                            </div>
                        </button>
                        <AnimateHeight duration={300} height={currentMenu === item.label.toLowerCase() ? 'auto' : 0}>
                            <ul className="sub-menu text-gray-500">
                                {item.children.map((child: any, idx: any) => (
                                    <li key={idx}>
                                        <NavLink to={child.link}>{t(child.label)}</NavLink>
                                    </li>
                                ))}
                            </ul>
                        </AnimateHeight>
                ...
                return (
                    <li key={index} className="menu nav-item">
                        <NavLink to={item.link} className={`${currentMenu === item.label.toLowerCase() ? 'active' : ''} nav-link group w-full`} onClick={() => toggleMenu(item.label.toLowerCase())}>
                            <div className="flex items-center">
                                {item.icon}
                                <span className="ltr:pl-3 rtl:pr-3 text-black dark:text-[#506690] dark:group-hover:text-white-dark">{t(item.label)}</span>
                            </div>
                ...
                <div className="bg-white dark:bg-black h-full">
                    <div className="flex justify-between items-center px-4 py-3">
                        <NavLink to="/" className="main-logo flex items-center shrink-0">
                            <img className="w-8 ml-[5px] flex-none" src="/minesphere.png" alt="logo" />
                            <span className="text-2xl ltr:ml-1.5 rtl:mr-1.5 font-semibold align-middle lg:inline dark:text-white-light">{t('MineSphere')}</span>
                        </NavLink>
