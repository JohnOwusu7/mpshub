...
const BlankWithHeader: React.FC = () => {
  ...
  const handleSignOut = () => {
    // Perform logout actions here (clear localStorage, etc.)
    dispatch(logout());
    navigate("/auth");
    showMessage({ message: 'Logout successful', success: true });
  };
                ...
                <div className="relative bg-white flex w-full items-center px-5 py-2.5 dark:bg-black">
                    <div className="horizontal-logo flex lg:hidden justify-between items-center ltr:mr-2 rtl:ml-2">
                        <Link to="/" className="main-logo flex items-center shrink-0">
                            <img className="w-8 ltr:-ml-1 rtl:-mr-1 inline" src="/minesphere.png" alt="logo" />
                            <span className="text-2xl ltr:ml-1.5 rtl:mr-1.5  font-semibold  align-middle hidden md:inline dark:text-white-light transition-all duration-300">MineSphere</span>
                        </Link>
                        <button
                            type="button"
                    ...
                    <a href='/dashboard'>
                    <IconArrowBackward className='ml-10 scale: h-12 w-12 color-green'/>
                    </a>
                    ...
                    <div className="sm:flex-1 ltr:sm:ml-0 ltr:ml-auto sm:rtl:mr-0 rtl:mr-auto flex items-center space-x-1.5 lg:space-x-2 rtl:space-x-reverse dark:text-[#d0d2d6]">
                        ...
                        <div className="dropdown shrink-0 flex">
                            <Dropdown
                                ...
                                button={<img className="w-9 h-9 rounded-full object-cover saturate-50 group-hover:saturate-100" src="/assets/images/auth/profile.png" alt="pic" />}>
                                <ul className="text-dark dark:text-white-dark !py-0 w-[230px] font-semibold dark:text-white-light/90">
                                    <li>
                                        <div className="flex items-center px-4 py-4">
                                            <img className="rounded-md w-10 h-10 object-cover" src="/assets/images/auth/profile.png" alt="userProfile" />
                                            <div className="ltr:pl-4 rtl:pr-4 truncate">
                                                <h4 className="text-base">
                                                {user.firstName}
                                                    <span className="text-xs bg-success-light rounded text-success px-1 ltr:ml-2 rtl:ml-2">{(user as any).roleName ?? (user as any).role ?? ''}</span>
                                                </h4>
                                    ...
                                    <li className="border-t border-white-light dark:border-white-light/10">
                                        <button type='button' className="text-danger !py-3" onClick={handleSignOut}>
...
export default BlankWithHeader;
